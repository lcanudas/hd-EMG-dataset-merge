...
def include_metadata(dado, pathcwd, database_id, **kwargs):
    ...
    dado['speed'] = kwargs['speed'] if 'speed' in kwargs.keys() else np.nan
    dado['speed'] = dado['speed'].astype('category')
    dado['session'] = kwargs['session'] if 'session' in kwargs.keys() else np.nan
    ...
    dado['original electrode distance (mm)'] = dado['original electrode distance (mm)'].astype('category')
    ...
    dado['electrode distance (mm)'] = dado['original electrode distance (mm)'].astype('category')
    ...
    if os.path.exists(pathcwd+path+'SubjectsDescription.txt'):
        ...
        for column in list(meta_subjects.columns)[1:]:
            if column.find('circumference') < 0 and column.find('length') < 0 :
                dado[column.lower()] = meta_subjects[column].values[0]
                dado[column.lower()] = dado[column.lower()].astype('category')
